...
import client.views.UserListPanel;
...
public class ClientUI extends JFrame implements IClientEvents, ICardControls {
    ...
    private void loadChatHistory(){
        List<String> chatHistory = historyManager.getChatHistory();
        // Notify ChatPanel to load the chat history
        chatPanel.onHistoryLoad(chatHistory);
    }
    ...
    private void addToChatHistory(String message) {
        historyManager.addMessage(myId, "You", message);
    }
    ...
    public void onMessageReceive(long clientId, String message) {
        if (currentCard.ordinal() >= Card.CHAT.ordinal()) {
            ...
            chatPanel.addText(String.format("%s: %s", clientName, message));
            addToChatHistory(String.format("%s: %s", clientName, message));
            // userListPanel.updateLastMessageSender(clientId);
        }
    }
